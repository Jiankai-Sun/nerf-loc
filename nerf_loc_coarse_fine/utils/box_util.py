...
from utils.objectron_iou import IoU, Box
...
def polygon_clip(subjectPolygon, clipPolygon):
    ...
    cp1 = clipPolygon[-1]
    # print('subjectPolygon: {}, clipPolygon: {}'.format(len(subjectPolygon), len(clipPolygon)))
    for i, clipVertex in enumerate(clipPolygon):
        cp2 = clipVertex
...
def box3d_iou(corners1, corners2):
    ...
    corners1 = np.concatenate((np.array([[1., 1., 1.]]), corners1), 0)
    corners2 = np.concatenate((np.array([[1., 1., 1.]]), corners2), 0)
    # print('corners1, corners2: ', corners1, corners2)
    prediction = Box(corners1)
    annotation = Box(corners2)
    ...
    # # print(rect1, rect2)
    # # print(inter, inter_area)  # None, 0.0
    # # print(inter_vol, vol1, vol2, ymax, ymin)
    return iou, iou_2d
...
def generalized_box3d_iou_tensor(
    ...
    """
    # print('corners1.shape: {}, corners2.shape: {}'.format(corners1.shape, corners2.shape))
    # corners1.shape: [8, 128, 8, 3], corners2.shape: [8, 1, 8, 3]
    assert len(corners1.shape) == 4
    assert len(corners2.shape) == 4
    assert corners1.shape[2] == 8
    assert corners1.shape[3] == 3
