...
class Matcher(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        # torch.Size([8, 64, 1]) torch.Size([8, 64, 1])
...
class SetCriterion(nn.Module):
    ...
    def loss_coord(self, outputs, targets, assignments):
        gt_box_coord = targets["gt_box_corners"]
        ...
        if targets["num_boxes_replica"] > 0:
            ...
            coord_loss /= targets["num_boxes"]
        else:
            coord_loss = torch.zeros(1, device=pred_box_coord.device).squeeze()
    ...
    def single_output_forward(self, outputs, targets):
        ...
        if self.dataset_config.version == 'v1_csa':
        ...
        for k in self.loss_weight_dict:
            if self.loss_weight_dict[k] > 0:
                ...
                print('final_loss: {}, losses[k.replace("_weight", "")]: {}, k: {}'.format(final_loss, losses[k.replace("_weight", "")], k))
