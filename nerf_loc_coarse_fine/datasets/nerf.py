...
class NerfDetectionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        semantic_class = np.asarray([0] * counter)
        try:
            bboxes = data['csa_bbox']
        except:
            bboxes = np.zeros((1 * counter, 8))

        poses_arr = np.load(os.path.join(video_path, "poses_bounds.npy"))
        ...
        if self.split_set in ['train']:  # , 'val']:
            target_bboxes_semcls[0: counter] = semantic_class
        else:
            target_bboxes_semcls[0: counter] = semantic_class
