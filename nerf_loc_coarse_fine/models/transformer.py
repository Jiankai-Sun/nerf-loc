...
class TransformerDecoder(nn.Module):
    ...
    def __init__(self, decoder_layer, num_layers, norm_fn_name="ln",
        ...
        self.num_layers = num_layers
        self.norm = None
        if norm_fn_name is not None and not self.layers[0].use_mlp:
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward_pre(self, src,
        ...
        # print(src.shape, src_mask, pos)  # torch.Size([8, 128, 256]) None None
        if self.use_mlp:
            src = self.encoder_mlp(torch.cat([src], dim=-1))
            attn_weights = None
        else:
            src2 = self.norm1(src)
            value = src2
    ...
    def extra_repr(self):
        st = ""
        if  not self.use_mlp and hasattr(self.self_attn, "dropout"):
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead=4, dim_feedforward=256,
                 ...
                 norm_fn_name="ln", use_mlp=False):
        super().__init__()
        self.use_mlp = use_mlp
        self.normalize_before = normalize_before
        self.attn_type = 'self_attn'  # orig_attn
        if not self.use_mlp:
            if dropout_attn is None:
                dropout_attn = dropout
            self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
            if not self.attn_type == 'self_attn':
    ...
    def forward_pre(self, tgt, memory,
        ...
        if self.use_mlp:
            ...
            if self.attn_type == 'orig_attn':
                # Original Decoder: Self_Attn + Multi_Attn
                tgt2 = self.norm1(tgt)
